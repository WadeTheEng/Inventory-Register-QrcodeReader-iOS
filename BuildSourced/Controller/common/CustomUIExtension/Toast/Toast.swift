...
public extension UIView {
    ...
    public func makeSuccessToast(_ message: String) {
        var style = ToastStyle()
        style.messageColor = UIColor.white
        style.backgroundColor = UIColor(named: .AppBlue)
        style.cornerRadius = 20
        self.makeToast(message, duration: ToastManager.shared.duration, position: ToastManager.shared.position, style: style)
    }
    
    public func makeFailedToast(_ message: String) {
        var style = ToastStyle()
        style.messageColor = UIColor.white
        style.backgroundColor = UIColor(named: .AppRed)
        style.cornerRadius = 20
        self.makeToast(message, duration: ToastManager.shared.duration, position: ToastManager.shared.position, style: style)
    }
    ...
    */
    public func toastViewForMessage(_ message: String?, title: String?, image: UIImage?, style: ToastStyle) throws -> UIView {
        ...
        //if let image = image {
        imageView = UIImageView(image: UIImage(named:"btn_close"))
        ...
        let wrapperWidth = max((imageRect.size.width + (style.horizontalPadding * 2.0)), (longerX + longerWidth + style.horizontalPadding * 2.0 + imageRect.size.width))
        let wrapperHeight = max((messageRect.origin.y + messageRect.size.height + style.verticalPadding), (imageRect.size.height + (style.verticalPadding * 2.0)))
        
        wrapperView.frame = CGRect(x: 0.0, y: 0.0, width: wrapperWidth, height: wrapperHeight)
        imageRect.origin.x = wrapperView.frame.size.width - style.horizontalPadding - imageRect.size.width
